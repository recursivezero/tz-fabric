...
export default function Composer({
    ...
    // example: parent logic to handle chip actions
    const handleChipAction = (actionId: string, opts?: { name?: string }) => {
        // IMAGE-only analysis chips: set input and immediately send
        if (actionId === "image:analyze_short") {
            onChange("Analyze this image (short).");
            return;
        }
        if (actionId === "image:analyze_long") {
            onChange("Analyze this image (long).");
            return;
        }
    };
                ...
                <SuggestionChips
                    ...
                    onAction={(actionId, opts) => {
                        if (typeof onChipAction === "function") {
                            ...
                            handleChipAction(actionId, opts);
                        }
                    }}
                />
            </div>
