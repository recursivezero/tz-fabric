...
const DEFAULT_IMAGE = "/dummy_product.jpg";
...
export default function ContentGrid() {
  ...
  // Track broken images without removing their cards
    ...
    (async () => {
      ...
      try {
        ...
        if (!ignore) {
          ...
          // Clear broken-image state when loading a new page
  ...
  function pickDisplayName(item: MediaItem) {
    ...
    // Prefer the actual uploaded/stored filename.
  ...
  const cleanName = (filename: string) => {
    ...
    const normalized = withoutExtension
      .replace(
        /(?:[_\s-]?20\d{6}T\d{6})(?:[_\s-]?[a-z0-9]{4,})?$/i,
        "",
      )
    ...
    <div className="grid-page">
      ...
      <div className="grid-header">
        <div className="grid-left">
          ...
          <span className="grid-count-inline">
            ({total})
          </span>
        </div>

        {mode === "all" && items.length > 0 && (
      ...
      <div className="media-grid">
        {items.map((item) => {
          ...
          const imageSrc = badImages.has(src)
            ? DEFAULT_IMAGE
            : src;
              ...
              <figure className="media-thumb">
                <div className="img-wrapper">
                  <img
                    ...
                    decoding="async"
                    onError={() => markBad(src)}
                    onClick={() => openLightbox(imageSrc, caption)}
                  />
