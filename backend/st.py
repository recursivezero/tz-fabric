...
st.set_page_config(page_title="Aadhaar Extractor", page_icon="🪪")
st.title("🪪 Aadhaar Card Extractor")

side = st.radio("Select Aadhaar side", ["front", "back"])
uploaded = st.file_uploader("Upload Aadhaar image", ["jpg", "jpeg", "png"])

@st.cache_resource
def load_extractor():
    return AadhaarCardExtractor()
...
if uploaded:
    ...
    with st.spinner("Extracting..."):
        result, raw, cleaned = extractor.extract_data(path, side)
    ...
    with st.expander("CLEANED OCR"):
        for _, t, c in cleaned:
            st.write(f"{t} ({c:.2f})")
