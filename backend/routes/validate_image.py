...
def _is_close_up_local(metrics: Optional[dict]) -> bool:
    if not metrics:
        return False
    count = 0
    if metrics.get("lap_var", 0.0) >= LAP_VAR_TH:
        count += 1
    if metrics.get("edge_density", 0.0) >= EDGE_DENSITY_TH:
        count += 1
    if metrics.get("patch_std_mean", 0.0) >= PATCH_STD_TH:
        count += 1
    return count >= 2
    ...
    try:
            ...
            if local_metrics_raw.get("lap_var", 0.0) < 20:
                ...
                _cache_set(
                    img_hash,
                    {
                        "verdict": "invalid",
                        "reason": reason,
                        "meta": {"metrics": local_metrics_raw},
                    },
                )
                return JSONResponse(
                    content={
        ...
        if parsed and isinstance(parsed, dict):
            ...
            model_meta.update(
                {
                    "parsed_model": parsed,
                    "texture_visible": texture_visible_flag,
                    "texture_confidence": model_texture_conf,
                    "pattern_full_motif": pattern_full_motif,
                }
            )
        ...
        if verdict == "invalid":
            if texture_visible_flag is True or (
                ...
                if not mentions_person and not face_found:
                    ...
                    model_meta["override"] = {
                        "by_texture_conf": True,
                        "model_texture_conf": model_texture_conf,
