...
def topk_search(collection: Collection, embedding: list[float], k: int = 10) -> dict[str, Any]:
    topN = min(max(k * 10, 100), 500)

    res = collection.query(query_embeddings=[embedding], n_results=topN, include=["metadatas", "distances"])
